...
In this example, we estimate the profile of chromosome 8 of two samples that are contained in
an {\tt oligoSnpSet} object. We load the data that are contained in the package {\tt SNPchip}. 
...
The object {\tt sample.snpset} contains the data of five HapMap samples and we want to analyze the third and the fourth samples.
 Since the samples should not have 
...
<<echo=TRUE,print=FALSE>>=
r <- estProfileWithMBPCRforOligoSnpSet(sample.snpset, sampleToBeAnalyzed=3:4, chrToBeAnalyzed=8, maxProbeNumber=1000, ifLogRatio=0, rhoSquare= 0.0889637)
@
