...
In this example, we estimate the profile of chromosome 1 of a sample that is contained in
...
The object {\tt sample.snpset} contains the data of five HapMap samples and we want to analyze the third one.
