...
estProfileWithMBPCRforOligoSnpSet <- function(sampleData, sampleToBeAnalyzed, chrToBeAnalyzed, maxProbeNumber, ifLogRatio=1, rhoSquare=NULL, kMax=50, nu=NULL, sigmaSquare=NULL, typeEstRho=1, regr=NULL)
 {
    if (class(sampleData) !="oligoSnpSet" || (length(assayData(sampleData)$copyNumber) == 0 | length(featureData(sampleData)$chromosome) == 0 | length(featureData(sampleData)$position) == 0)) {
        ...
        resultPC <- new("oligoSnpSet", calls=assayData(sampleData)$calls, callsConfidence =assayData(sampleData)$callsConfidence, copyNumber=cnMatrix, cnConfidence = matrix(NA, length(chr), dim(log2ratios)[2]),annotation = annotation(sampleData) ,phenoData = phenoData(sampleData), featureData = featureData(sampleData))
        if (length(regr) == 0) {
            return(list(estPC=resultPC))
        } else {
            resultRegr <- new("oligoSnpSet", calls=assayData(sampleData)$calls, callsConfidence =assayData(sampleData)$callsConfidence, copyNumber=regrMatrix, cnConfidence = matrix(NA, length(chr), dim(log2ratios)[2]),annotation = annotation(sampleData) ,phenoData = phenoData(sampleData), featureData = featureData(sampleData))
